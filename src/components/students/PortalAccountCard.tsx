...
import {
  ...
  useSendPortalCredentials,
  useSetPortalAccountActive,
} from "@/hooks/usePortalAccount";
import type { PortalCredentials } from "@/api/portal-account.api";
import type { Student } from "@/types/student";

const formatDate = (iso: string | null) =>
  iso ? new Date(iso).toLocaleString("fr-FR", { dateStyle: "short", timeStyle: "short" }) : null;

/** Ouvre une fenêtre d'impression avec la fiche d'identifiants à remettre au parent. */
function printCredentials(creds: PortalCredentials, schoolName?: string) {
...
<html lang="fr"><head><meta charset="utf-8"><title>Identifiants - ${creds.studentName}</title>
<style>
  ...
  <h1>Accès au portail parent</h1>
  <div class="sub">${schoolName ?? "EcoleNet"} — élève : <strong>${creds.studentName}</strong></div>
  <table>
  ...
  <p class="note">
    ...
    Cet identifiant est propre à cet enfant : si vous avez plusieurs enfants dans l'établissement,
    vous disposez d'un identifiant par enfant.
  </p>
  <div class="cut"></div>
</body></html>`);
  win.document.close();
...
export function PortalAccountCard({ student, schoolName }: Props) {
  const { data: account, isLoading } = usePortalAccount(student.id);
  const createMutation = useCreatePortalAccount(student.id);
  const resetMutation = useResetPortalPassword(student.id);
  const sendMutation = useSendPortalCredentials(student.id);
  const activeMutation = useSetPortalAccountActive(student.id);
  ...
  const onSendByEmail = () =>
    sendMutation.mutate(undefined, {
      onSuccess: (creds) =>
        creds.emailSent
          ? toast.success(`Identifiants envoyés à ${creds.parentEmail}`)
          : toast.warning(
    ...
    <>
      <div className="rounded-xl border border-border/50 bg-card p-5 shadow-sm">
        ...
        {isLoading ? (
          ...
          <div className="flex items-start gap-2 text-sm text-amber-700 bg-amber-50 rounded-lg p-3">
            ...
            <span>
              Cet élève n'a pas de matricule : l'identifiant de connexion ne peut pas être généré.
            </span>
          </div>
        ) : !account?.exists ? (
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Aucun compte pour cet élève. L'identifiant sera son matricule{" "}
          ...
          <div className="space-y-3 text-sm">
            ...
            <Row label="Mot de passe">
              {account.passwordVisible ? (
                ...
                <span className="text-muted-foreground italic">
                  Défini par le parent — non consultable
                </span>
              )}
            </Row>
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              ...
              {account.parentEmail && account.passwordVisible && (
                <Button
                  variant="outline"
                  size="sm"
                  className="gap-1.5"
      ...
      <Dialog open={confirmReset} onOpenChange={setConfirmReset}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              L'ancien mot de passe cessera immédiatement de fonctionner. Le parent devra utiliser le
              nouveau pour se connecter.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={!!credentials} onOpenChange={(open) => !open && setCredentials(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Identifiants du portail</DialogTitle>
            <DialogDescription>
              À remettre au parent de {credentials?.studentName}.
            </DialogDescription>
          </DialogHeader>
          ...
          {credentials?.emailSent ? (
            <p className="text-xs text-emerald-700 flex items-center gap-1.5">
              <Mail className="h-3.5 w-3.5" />
              Envoyés par email à {credentials.parentEmail}
            </p>
          ) : credentials?.parentEmail ? (
            <p className="text-xs text-amber-700 flex items-start gap-1.5">
              <MailX className="h-3.5 w-3.5 mt-0.5 shrink-0" />
              <span>
                L'envoi à {credentials.parentEmail} n'a pas abouti. Imprimez la fiche ou
                communiquez les identifiants directement.
              </span>
            </p>
          ) : (
            <p className="text-xs text-amber-700 flex items-start gap-1.5">
              <MailX className="h-3.5 w-3.5 mt-0.5 shrink-0" />
              <span>
                Aucun email parent sur la fiche — remettez ces identifiants en main propre.
              </span>
            </p>
          )}
